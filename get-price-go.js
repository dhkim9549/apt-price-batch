...
async function insertPrc(fileNm) {
  ...
  for await (const line of file.readLines()) {
    ...
    const basisYr = apt.basisYr;

    await appendFile("r-list-go-output.txt", "\n" + w[1], "utf8");
    ...
    const stnmAddr2 =
      stnm_addr_w[stnm_addr_w.length - 2] +
      " " +
      stnm_addr_w[stnm_addr_w.length - 1];
    ...
    if (aptPrcAr.length > 1) {
      ...
      aptPrcAr = aptPrcAr.filter(
        (element) => element.aptNm.indexOf(aptNm.slice(0, 2)) >= 0,
      );
    }

    if (aptPrcAr.length == 0) {
      aptPrcAr = await collection
        .find({
          basisYr: basisYr,
          stnmAddr2: stnmAddr2,
          hoNm: { $in: [hoNm, hoNm2] },
    ...
    await appendFile("r-list-go-output.txt", "," + basisYr + "," + price, "utf8");
